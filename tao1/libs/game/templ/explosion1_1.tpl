...
<head>
    <style type="text/css">
        ...
        canvas     { height: 100%; width: 100%; padding: 0; margin: 0;}
        #viseur    { position:absolute; top:50%; left:50%; margin-top:-37px; margin-left:-37px; }
{#        #guiCanvas { position:absolute; top:0; pointer-events: none; }#}
    </style>
...
Player.prototype = {
    ...
    _initCamera : function() {
        ...
        cam.keysUp =    [38, 87]; // Z
        cam.keysDown =  [40, 83]; // S
        cam.keysLeft =  [37, 65]; // Q
        cam.keysRight = [39, 68]; // D
...
Target = function(game, posX, posZ) {
    ...
    var _this = this;
    this.game.scene.registerBeforeRender(function() { _this.rotation.y += 0.01; });
};
Target.prototype = Object.create(BABYLON.Mesh.prototype);
Target.prototype.constructor = Target;
Target.prototype.explode = function() { this.dispose(); };
Weapon = function(game, player) {
    ...
    this.canFire = true;
    var scene = this.game.scene;  // The particle emitter
    ...
    particleSystem.particleTexture = new BABYLON.Texture("/static/game/babylon/gunshot_125.png", scene);
...
Weapon.prototype = {
    ...
    fire : function(pickInfo) {
        if (this.canFire) {
            if (pickInfo.hit && pickInfo.pickedMesh.name === "target") pickInfo.pickedMesh.explode();
            else {
...
Game = function(canvasId) {
    ...
    this.assets = {};
    var meshTask = this.loader.addMeshTask("gun", "", "/static/game/babylon/", "gun.babylon");
    meshTask.onSuccess = function(task) { _this._initMesh(task); };
    this.loader.onFinish = function (tasks) {
        ...
        window.addEventListener("keyup", function(evt) { _this.handleUserInput(evt.keyCode); });
    };
    this.loader.load();
...
Game.prototype = {
    _initScene : function(engine) {
        ...
        skybox.layerMask = 2;
        BABYLON.Engine.ShadersRepository = "/static/game/babylon/shaders/";
