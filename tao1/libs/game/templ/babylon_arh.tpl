...
{#            tasklmesh = loader.addMeshTask('name', "", "static/game/babylon/", "ufo3.babylon");#}
{#            tasklmesh = loader.addMeshTask('name', "", "static/game/babylon/", "q.babylon");#}

            tasklmesh = loader.addMeshTask(name, "", "static/game/babylon/", "int.babylon");
...
{#            var tasklmesh = loader.addMeshTask(name, "", "static/game/babylon/", "int.babylon");#}
{#            tasklmesh.onSuccess = function (task) {#}
...
var createScene = function () {
    ...
    // Ground
    var ground = BABYLON.Mesh.CreateGroundFromHeightMap("ground", "/static/game/babylon/water/Assets/heightMap.png", 90, 90, 70, 0, 10, scene, false);
...
{#            ws.send( JSON.stringify( { 'e':'shoot', 'id':player.id, 'pos':camera.position, 'dir':camera.cameraDirection } ) );           //ws.send( JSON.stringify( { 'e':'shoot', 'id':player.id, 'dir':camera.cameraDirection, 'pos':camera.position, 'trueDir':trueDirection } ) );#}
    ...
    var wsUri = (window.location.protocol=='https:'&&'wss://'||'ws://')+window.location.host+':8088';
...
{#            if (msg.id == localPlayer.id) logg('local messages');#}
...
{#        ws.send( JSON.stringify( { 'e':'shoot', 'id':player.id, 'dir':direction, 'pos':camera.position } ) );#}
        scene.registerBeforeRender(function () {
