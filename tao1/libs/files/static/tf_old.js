...
//            console.log('content',  content);
        ...
        function af(url, data, cb, pb){
            ...
            $.ajax({
                ...
                success: function(data) {
                    //data.remove();
                    //console.error('formd ------> ', formd);
                    //formd=[];
                    //formd.length=[];
                    //console.error('formd2 ------> ', formd);
                    if (cb) cb(data, false);
                },
...
//                if (!file.type.match(/image.*/)) return true;
            ...
            //af('/add_files', form, function(data, error){
            ////bef.click();
            ////if (error || data.result != 'ok') alert('Произошла ошибка. Возможно один или несколько файлов не отправились.');
            ////console.error('form ------> ', form);
            //    bef.remove();
            //    update();
            //    form=null;
