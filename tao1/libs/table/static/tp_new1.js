...
//    };
    ...
    window.dao.tableprocessor = function (parent, params){
        ...
        var element = parent;			/** {jQuery} родительский контейнер */
...
//        var cursor = {  v: 0, h: 0 };
        ...
        var toolbar = $('<div style="position:absolute; top:0px; left: 0px; right:0px; width:auto; height:38px;" id="'+options.id+'_'+options.doc_id+'_toolbar" '+
            'class="ui-widget-header ui-corner-all button_div"></div>').appendTo(element);
        ...
        var def_b = $('<div style="margin-left:4px;" class="btn-group"></div>').appendTo(toolbar);
        var extra_b = $(
            '<div class="btn-group"> ' +
                '<a title="" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" >' +
                    '<i class="icon-star"></i> <span style="color:#59b; padding-right:4px;">Ёще</span><span class="caret"></span>' +
                '</a>' +
                ' <ul class="dropdown-menu">' +
                    '<li class="duplicate_all"><a rel="tooltip" data-toggle="dropdown" href="#"><i class="icon-mail-reply-all"></i> Дублировать полностью</a></li>' +
            '</ul>' +
            '</div>').appendTo(toolbar);
        var field_b = $(
            '<div class="btn-group"> ' +
                '<a title="" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" ><i class="icon-wrench"></i> <span style="color:#59b; padding-right:4px;">Поля</span><span class="caret"></span></a>' +
                ' <ul class="dropdown-menu"></ul>' +
            '</div>').appendTo(toolbar);

        for(var index in options.actions){
            var t_button;
            if(act[index].type_button == 'default')
                t_button = $('<a style=""  rel="tooltip" title="'+act[index].hint+'" class="btn btn-default btn-sm" data-toggle="modal" href="#myModal"><i class="'+act[index].icon+'"></i>'+act[index].title+'</a>').appendTo(def_b);//(toolbar);
            else if(act[index].type_button == 'extra')
                t_button = $('<li><a rel="tooltip" title="" data-toggle="dropdown" href="#">' +
                                '<i class="'+act[index].icon+'"></i> '+act[index].title +
                            '</a></li>').appendTo( extra_b.find('ul'));
            else if(act[index].type_button == 'field')
                t_button = $('<li><a rel="tooltip" title="" data-toggle="dropdown" href="#">' +
                                '<i class="'+act[index].icon+'"></i> '+act[index].title +
                            '</a></li>').appendTo( field_b.find('ul'));
            else t_button = $('<a style=""  rel="tooltip" title="'+act[index].hint+'" class="btn btn-default btn-sm" data-toggle="modal" href="#myModal"><i class="'+act[index].icon+'"></i>'+act[index].title+'</a>').appendTo(def_b);
            var opt = act[index];
            var aaa = opt['action'].split(/\./);
//            var name_func = eval(aaa[1], this);
            var name_func = eval(opt['action']);
            _actions[aaa[1]] = name_func;
            if(!t_button) continue
            var a = new function() {
                var _opt = opt;
                var action = name_func;
                t_button.click(function(){  action.call(this, _opt);  });
            }
        }
        var toolbar_height = toolbar.outerHeight();

        var statusbar = $('<div style="text-align:left; bottom:0px; left:4px; right:4px; height:20px; position:absolute; font-size:12px;"></div>').appendTo(parent);
...
//        main.click(function(){ focus(); });
        ...
        var pages = $('<div class="cerp-ui-pages" style="top:0px; bottom:0px; right:0px; padding-right:12px; position:absolute; font-size:12px;"></div>').appendTo(statusbar);
        //			main.empty();
...
//        });
        ...
        //$('.grid.left, .grid.center, .grid.right').sortable();
        //$('.grid.left, .grid.center, .grid.right').disableSelection();
        //$('.grid.center, .grid.right').disableSelection();
        ...
        function init_event(){
            ...
            //    helper: function(e){
            //        var target = $(e.target);
            //        if (!target.is('.cell')) target = target.closest('.cell');
            //        row_id = target.attr('recid');
            //        var helper = $('<div class="ui-state-highlight ui-corner-all" style="z-index:9999; padding:8px;">['+row_id+']</div>').appendTo($("body"));
            //        is_tree_visible = _is_tree_visible();
            //        if (!is_tree_visible) _show_tree();
            //        return helper;
            //    },
            //    stop: function(e){ if (!is_tree_visible)  _hide_tree(); },
            ...
            main.on('click', '.icon-trash', function() { // удалить документ
                var ids = {};
            ...
            main.on('click', '.icon-folder-open', function() { // вызвать галерею
                file_manager.show( $(this).closest('.cell').attr('recid') );         return false;
            });
            main.on('click', '.icon-share-alt',   function() {  // дублировать документ
                crn.duplicate(     $(this).closest('.cell').attr('recid') );         return false;
            });
            main.on('click', '.icon-mail-reply-all', function() {  // дублировать документ полностью
            ...
            main.on('click', '.icon-edit',        function() { // редактировать
                crn.edit(          $(this).closest('.cell').attr('recid') );         return false;
            });
            main.on('click', '.icon-th-large',    function() { //просмотреть в базе
                crn.show_doc(      $(this).closest('.cell').attr('recid') );         return false;
            });
            main.on('click', '.icon-share',       function() { // просмотреть на сайте
                ...
                if (is_visible) {
                    ...
                    if (!row_hidden1.attr('is_loaded')) {
                        ...
                        for(var i in options.parts){
                            ...
                            $('<li><a href="#tabs-'+i+'-'+row_id+'">'+ part.title +'</a></li>').appendTo(ul);
                            var tp1 = $('<div id = "tabs-'+ i +'-'+row_id+'" style="position:absolute; left:0px; right:0px; top:30px; bottom:0px;"></div>').appendTo(div_tabs);
                            ...
                            }else if(part.type == 'comments'){
                                subs[i] = new dao.tree_processor(tp, null, {
                                    url: '/tree/data/'+part.id,
                                    id: part.id,
                                    check_tree:  true,
                                    is_add_link: false,
                                    is_full_id:  true,
...
//                    var child = cell.data('child');
                ...
                if (options.table_in_dialog && target.closest('.column').is('.cb')) {
                    ...
                    if( !$('.dl [recid="'+recid+'"]').length){
                        ...
                        $('<div recid="'+recid+'" style="display: inline-block; width: 350px; padding:0px 20px 0 20px">'+name+'' +
                            ...
                            '<i class="icon-remove del_ware_tid" style="color:red;"/></div></div>').appendTo(dialog);
                        return true;
                    }
                }
    ...
    //});
        ...
        function head_ (columns) {
            ...
            grid_center.empty();
            for(var res in options.columns){
                var opt  = options.columns[res];
                grid_center.columns[res] = make_column(res, opt, grid_center);
            }
        ...
        function make_column(res, opt, target){
            ...
            if(res == 'img' || res == 'act' || res == 'cb' || res == 'h'){}
                ...
                var btn_filter = $('<div class="btn-group">' +
                    '<div class="btn btn-default btn-sm show_filter" style="font-size:10px; padding:0px; margin:0px; line-height:16px;"><i class="icon-caret-down"></i></div>' +
...
//                make_column_filter(col.filter);
        ...
        var layout_bottom = $('<div style="position:absolute; height:20px; bottom:0px; left:0px; right:22px; cursor:default;"></div>').appendTo(main);
        ...
        var scroll = new dao.scroll(layout_aside, layout_bottom, {
            ...
            onscroll:function(left, top){
                for( var res in grid_center.columns) grid_center.columns[res].rows.scrollTop(top);
                for( var res in grid_left.columns) grid_left.columns[res].rows.scrollTop(top);
                grid_hidden.columns['h'].rows.scrollTop(top);
                for( var res in grid_right.columns) grid_right.columns[res].rows.scrollTop(top);
...
//        grid_center.scroll();
        ...
        if (options.has_tree) {
            ...
            tree_button = $('<div class="btn btn-info btn-sm" style="margin-left:25px;"><i class="icon-list-alt"></i> '+dao.ct('categories')+'</div>').appendTo(searchdiv_date);
        ...
        function make_column_filter(parent, c_filter){
            ...
            if (column.type == "select") {
                ...
                var div = $('<ul class="dropdown-menu filt_col_check" role="menu" style="padding:6px;">'+
                                "<li class='t'><span style='color:green; font-size:20px; cursor:pointer;'><i class='icon-ok'></i> Да</span></li>"+
                                "<li class='f'><span style='color:red;   font-size:20px; cursor:pointer;'><i class='icon-remove'></i> Нет</span></li>"+
                                "<li class='clean'><span style='color:gray; font-size:20px; cursor:pointer;'><i class='icon-eraser'></i> Очистить</span></li>"+
                            "</ul>").appendTo(parent);
//                div.hide();
...
//                parent.find(':radio[name="' + column_name + '_str"]').click(function(){
        ...
        function style_page_number(){
            ...
            pages.find('li').addClass('page_button ui-state-default');
            pages.find('.page_button').hover(function(){
                $(this).addClass('ui-state-hover');
            }, function(){
                $(this).removeClass('ui-state-hover');
            });
            pages.find('li:first').addClass('ui-corner-left');
            pages.find('li:last').addClass('ui-corner-right');
            pages.find('li.cerp-ui-pages-current').addClass('ui-state-active');
        ...
        function collapse_sub_rows (parent) {
            $('.column:first', grid_center).find('[parent="'+parent+'"]').each(function(i, v){
                var btn = $(v).find('.row_tree');
                if (btn.is('.expanded')) btn.click();
            });
        }
        ...
        // * Инициализация редактирования, редактирование ячейки
        function editCell(){
            var cell = $('.ui-state-active', grid_center);
            if(!cell.length) return undefined;
...
//                        dao.log('updatelist.draw');
        ...
        function pag(count, current){
            ...
            function pag_link(i) {
                var a = $('<a style="display:inline-table; padding:2px 10px;" href="#page'+i+'">  '+i+' </a>').appendTo(pages);
                if (i==current){ a.addClass('ui-state-active'); }
                else { a.addClass('ui-state-default'); }
            }
            function pag_dot() {
                var a = $('<span style="display:inline-table; padding:2px 10px;">  ... </span>').appendTo(pages);
...
//            dao.log('updatelist.draw.left');
            make_left_html(data);
//            dao.log('updatelist.draw.right');
            make_right_html(data);
//            dao.log('updatelist.draw.hidden');
            make_hidden_html(data)
//            dao.log('updatelist.draw.other');
            ...
            function make_center_html(data){
                ...
                for(var ci in options.columns){
                    ...
                    for(var ri = 0; ri < rl; ri++){
                        ...
                        var cell = null;
                        var dl = row['doc'].length; // row['doc']  кол-во списков со словарями внутри в которых значения полей  [{},{},{}] [{},{},{}]
                        ...
                        if( first){
                            stl = 'text-align: left;';
                            //if( parent == '_'){
                            //    btn = '<i style="cursor:pointer; margin-right:6px; color:grey;" class="icon-move"></i>' + btn;
                                btn = '<i style="cursor:pointer; margin-right:6px; color:#8258FA;" class="icon-move"></i>' ;
                            //}
                            if( row['child'] && row['child'].length){
                                var exp = options.expanded  ? 'expanded' : 'collapsed';
...
//                    html += '<div class="fut" style="height:300px;"> &nbsp;</div>';
                    if(parent && parent != '_'){ //работаем с вложеными подстрочками
                        ...
                        aaa.after(html);
                        if (ctr>1) continue;
                        //$('[parent="'+parent+'"]:first').prepend("&nbsp;&nbsp;<i class='icon-ellipsis-vertical'></i>");  //плюсик или минус вставлять надо после этой штуки
                    } else grid_center.columns[ci].rows[0].innerHTML = html;
                        ...
                        if( ci == 'cb'){
                            var _cell = '<div class="row_button">' +
                                ...
                                if (options.parts && options.parts.length) {
                                    _cell += '<div class="button_div_icon_inner2 icon-plus-sign uiicon"></div>';
                                }
                            _cell += '</div>';
                ...
                for(var ci in columns){
                    ...
                    for(var ri = 0; ri < rl; ri++){
                        ...
                        if( ci == 'img'){
                            ...
                            _cell += '<div class="uiicon icon-folder-open"></div>';
                            _cell += '</div>';
                            cell = { 'edit_value':null, 'formatted': _cell };
//                            var _cell =
...
//                            cell = { 'edit_value':null, 'formatted': _cell };
                        ...
                        if( ci == 'act'){
                            ...
                            for (var index in options.actions) {
                                ...
                                var aaa = action['action'].split(/\./)[1];
                                var _opt  = JSON.stringify(action).split('"').join('&quot;');
                                _cell += (action['class'] && action['class']['inrow']) ? '<div class="row_button" action="'+aaa+'" options="'+_opt+'" title="'+action.hint+'"><div><i class="'+action.icon+'"></i></div></div>' : '';
                            }
                            cell = { 'edit_value':null, 'formatted': _cell };
                        }
                        if (!cell) { cell = { 'edit_value':null, 'formatted': '-' }; }
...
//            crn.show_doc(row_id);
        ...
        for (var i = 0; i < rows.length; i++) {
            Sortable.create(rows[i], {
                onEnd: function (/**Event*/evt) {
                    // если двигать вверх то срабатует next
                    // если двигать вниз то срботает   prev
        ...
        function sort_date($p, $n, doc_id){
            $.ajax({
                url: '/table/sort/date', // url: '/table/data/'+options.id,
                type: "POST", dataType:"json",
                data: {
                    proc_id: options.id,
                    doc_id: doc_id,
