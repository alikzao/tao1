...
{#                        <span>На модерации</span>#}
                    ...
                    {% set user=env.get_full_user(env.ct(item.title), True) %}
						    ...
						    {%   if not item.is_del %}
                                <div class="comm_title" >
                                    ...
                                    <div style="float: left;  ">
                                        {% if not env.len(user.att)  or not user.att[0]%}
...
{#	                                item.title -  id пользователя     #}
                                    ...
                                    {% set soc_name = env.ct( item.title) %}
                                    <div class="comm_name">
                                        {% if  soc_name.startswith('fb:')  %}
                                            <i class="icon-facebook-sign" style="font-size:18px;"></i>
                                            <b><a class="un" id_comm="{{item.id}}" target="_blank" href="http://facebook.com/{{ env.ct( item.title)[3:] }}" rel="nofollow"> {{ env.ct(item.name) }} </a></b>
                                        {% elif  soc_name.startswith('vk:')  %}
                                            <i class="icon-vk" style="font-size:18px;"></i>
                                            <b><a class="un" id_comm="{{item.id}}" target="_blank" href="http://vk.com/id{{ env.ct( item.title)[3:] }}" rel="nofollow"> {{ env.ct(item.name) }} </a></b>
                                        {% elif  soc_name.startswith('tw:')  %}
                                            <i class="icon-twitter-sign" style="font-size:18px;"></i>
                                            <b><a class="un" id_comm="{{item.id}}" target="_blank" href="http://twitter.com/account/redirect_by_id?id={{ env.ct( item.title)[3:] }}" rel="nofollow"> {{ env.ct(item.name) }} </a></b>
                                        {% elif  soc_name.startswith('gl:')  %}
                                            <i class="icon-google-plus-sign" style="font-size:18px;"></i>
                                            <b><a class="un" id_comm="{{item.id}}" target="_blank" href="http://plus.google.com/{{ env.ct( item.title)[3:] }}" rel="nofollow"> {{ env.ct(item.name) }} </a></b>
                                        {% elif  soc_name.startswith('ya:')  %}
                                            <i class="icon-sign-blank" style="font-size:18px;"><i>Y</i></i>
                                            <b><a class="un" id_comm="{{item.id}}" target="_blank" href="http://{{ env.ct( item.title)[3:] }}.ya.ru/" rel="nofollow"> {{ env.ct(item.name) }} </a></b>
                                        {% else  %}
                                            {{ env.ct( item.title) }}
                                        {% endif  %}
                                        {% if user.id != 'user:ariru' %}
                                            {% set user_rate = (0 if not user.doc.rate else env.float(user.doc.rate)) %}
...
{#                                    <div class="comm_date" itemprop="datePublished" datetime="{{ item.date }}">#}
                                    <time class="comm_date" itemprop="commentTime" datetime="{{  env.format_date(item.date, '%Y-%m-%dT%H:%M:%S')}}">
                                        {{ item.date }}
                                    </time>
                                </div>
                                <div id_comm="{{item.id}}" itemprop="commentText" class="comm_comment ">{{ env.ct( item.descr )}}</div>
                                <div  class="comm_answer ui-corner-all " style="cursor: pointer; padding-bottom:10px;"> <a href='#'>{{ env.ct('reply') }}</a></div>
			...
			<div comm_cont="__" style="visibility: hidden; height:0px;">
                <div class="comm_editor" style="margin: 20px 10px; border: 1px solid #ddd; background: rgba(255,255,255,.4);">
                    ...
                    <div class="comm_title">{{ env.ct( 'name') }}: <b data='name'></b> </div>
                    <div class="easewig" ></div>
                    <div class=""style="text-align: right; padding: 5px 50px;"><div class="comm_create btn btn-primary" style="cursor: pointer; width:120px; font-size:14px; "> {{ env.ct( 'send') }}</div></div>
                    <input class="hide_inp"/>
                </div>
			</div>
	...
	{% if env.len(tree.children ) > 2 %} <div class="comment_button"></div> {% endif %}
{% endif %}


{#{%  if env.user.is_admin or item.title == env.user.id  %}#}
