...
function RTCMultiConnection(room) {
...
//                channel = config.channel || self.channel || 'default-channel';
                var socket  = new WebSocket("ws://ariru.py.mongo:9876/broadcast?room="+this.room+"&user_id="+this.user_id);
    ...
    function init() {
        ...
        self.config = {
            ...
            onChannelMessage: function (data) {
                console.log('onChanellMess', data);
...
function getUserMedia(options) {
    ...
    n.getMedia(options.constraints || { audio: true, video: video_constraints }, streaming, options.onerror || function (e) { console.error(e); });
...
function RTCMultiSession(config) {
    var session = config.session || Session.AudioVideo,
        ...
        self = { id: uniqueToken(), sessionid: config.channel },
        channels = '--',
        isbroadcaster,
...
//        if (typeof response.data)
        if (response.userid == self.id) return;
        console.log('response', response);

        console.log('newsess', isAcceptNewSession, response.sessionid, response.userid, isAcceptNewSession && response.sessionid && response.userid);
        if (isAcceptNewSession && response.sessionid && response.userid)
            if (config.onNewSession) config.onNewSession(response);

        console.log('newpart', response.newParticipant);
        if (response.newParticipant){
            onNewParticipant(response.newParticipant);
            console.log('part', response);
        }

        console.log('last', response.userid, response.targetUser, self.id, response.participant, channels.indexOf(response.userid), -1, response.userid && response.targetUser == self.id && response.participant && channels.indexOf(response.userid) == -1);
    ...
    function openSubSocket(_config) {
        ...
        var socket = config.openSignalingChannel(socketConfig),
            isofferer = _config.isofferer, isGotRemoteStream, inner = {}, peer,
        ...
        function initPeer(offerSDP) {
            ...
            if (session.indexOf('data') === -1) peerConfig.onChannelMessage = null;
            if (session.isAudio()) {
                /* OfferToReceiveVideo MUST be false for audio-only streaming */
                peerConfig.constraints = { optional: [], mandatory: { OfferToReceiveAudio: true, OfferToReceiveVideo: false } };
            }
...
//        console.log('defaultSocket', defaultSocket);
        defaultSocket.send({
